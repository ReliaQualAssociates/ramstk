...
class TestCreateControllers:
    ...
    def test_data_manager(self):
        ...
        assert pub.isSubscribed(DUT._do_insert_opload, "request_insert_pof_opload")
        assert pub.isSubscribed(DUT._do_insert_opstress, "request_insert_pof_opstress")
        assert pub.isSubscribed(
            DUT._do_insert_testmethod, "request_insert_pof_testmethod"
        )
        ...
        assert pub.isSubscribed(DUT.do_set_attributes, "request_set_pof_attributes")
...
class TestDeleteMethods:
    ...
    def on_fail_delete_pof(self, error_message):
        assert error_message == (
            "_do_delete: Attempted to delete non-existent PoF record with " "ID 300."
        )
    ...
    def test_do_delete_test_method(self, test_program_dao):
        ...
        pub.unsubscribe(self.on_succeed_delete_test_method, "succeed_delete_pof")
...
class TestInsertMethods:
    ...
    def on_fail_insert_opstress(self, error_message):
        assert error_message == (
            "An error occurred when attempting to add an "
    ...
    def test_do_insert_opload(self, test_program_dao):
        ...
        DUT._do_insert_opload("5.2")

        assert isinstance(DUT.tree.get_node("5.2.5").data["opload"], RAMSTKOpLoad)
        assert DUT.tree.get_node("5.2.5").data["opload"].load_id == 5
        assert (
            DUT.tree.get_node("5.2.5").data["opload"].description == "New "
            "Operating Load"
        )
    ...
    def test_do_insert_opstress(self, test_program_dao):
        ...
        pub.subscribe(self.on_succeed_insert_opstress, "succeed_insert_opstress")
        ...
        pub.unsubscribe(self.on_succeed_insert_opstress, "succeed_insert_opstress")
    ...
    def test_do_insert_test_method(self, test_program_dao):
        ...
        pub.subscribe(self.on_succeed_insert_test_method, "succeed_insert_test_method")
        ...
        pub.unsubscribe(
            self.on_succeed_insert_test_method, "succeed_insert_test_method"
        )
    ...
    def test_do_insert_test_method_no_cause(self, test_program_dao):
        ...
        pub.unsubscribe(self.on_fail_insert_test_method, "fail_insert_test_method")
...
class TestGetterSetter:
    ...
    def on_succeed_get_pof_tree(self, tree):
        ...
        assert isinstance(tree.get_node("5.2").data["mechanism"], RAMSTKMechanism)
    ...
    def test_do_get_mechanism_attributes(self, test_program_dao):
        ...
        assert isinstance(DUT.tree.get_node("5.2").data["mechanism"], RAMSTKMechanism)
    ...
    def test_do_get_opload_attributes(self, test_program_dao):
        ...
        pub.subscribe(self.on_succeed_get_opload_attrs, "succeed_get_opload_attributes")

        DUT = dmPoF()
        DUT.do_connect(test_program_dao)
        DUT.do_select_all({"revision_id": 1, "hardware_id": 1})
        DUT.do_get_attributes("5.2.2", "opload")

        assert isinstance(DUT.tree.get_node("5.2.2").data["opload"], RAMSTKOpLoad)

        pub.unsubscribe(
            self.on_succeed_get_opload_attrs, "succeed_get_opload_attributes"
        )
    ...
    def test_do_get_opstress_attributes(self, test_program_dao):
        ...
        attributes on success."""
        pub.subscribe(
            self.on_succeed_get_opstress_attrs, "succeed_get_opstress_attributes"
        )

        DUT = dmPoF()
        DUT.do_connect(test_program_dao)
        DUT.do_select_all({"revision_id": 1, "hardware_id": 1})
        DUT.do_get_attributes("5.2.2.2.s", "opstress")

        assert isinstance(
            DUT.tree.get_node("5.2.2.2.s").data["opstress"], RAMSTKOpStress
        )
    ...
    def test_do_get_test_method_attributes(self, test_program_dao):
        ...
        on success."""
        pub.subscribe(
            self.on_succeed_get_test_method_attrs, "succeed_get_test_method_attributes"
        )

        DUT = dmPoF()
        DUT.do_connect(test_program_dao)
        DUT.do_select_all({"revision_id": 1, "hardware_id": 1})
        DUT.do_get_attributes("5.2.2.2.t", "testmethod")

        assert isinstance(
            DUT.tree.get_node("5.2.2.2.t").data["testmethod"], RAMSTKTestMethod
        )
    ...
    def test_do_set_opload_attributes(self, test_program_dao):
        ...
        DUT.do_set_attributes(
            node_id=["5.2.2", -1], package={"damage_model": "Fancy math model"}
        )
        DUT.do_set_attributes(
            node_id=["5.2.2", -1], package={"description": "Jared Kushner"}
        )
        assert DUT.do_select("5.2.2", table="opload").description == "Jared Kushner"
        assert DUT.do_select("5.2.2", table="opload").damage_model == (
            "Fancy math model"
        )
    ...
    def test_do_set_opstress_attributes(self, test_program_dao):
        ...
        DUT.do_set_attributes(
            node_id=["5.2.2.2.s", -1], package={"load_history": "Waterfall histogram"}
        )
        DUT.do_set_attributes(
            node_id=["5.2.2.2.s", -1], package={"description": "Lock and chain"}
        )
        assert (
            DUT.do_select("5.2.2.2.s", table="opstress").description == "Lock and chain"
        )
        assert DUT.do_select("5.2.2.2.s", table="opstress").load_history == (
            "Waterfall histogram"
        )
    ...
    def test_do_set_test_method_attributes(self, test_program_dao):
        ...
        DUT.do_set_attributes(
            node_id=["5.2.2.2.t", -1], package={"description": "Kick his ass"}
        )
        DUT.do_set_attributes(
            node_id=["5.2.2.2.t", -1], package={"remarks": "Doyle Rowland"}
        )
        assert (
            DUT.do_select("5.2.2.2.t", table="testmethod").description == "Kick his ass"
        )
        assert DUT.do_select("5.2.2.2.t", table="testmethod").remarks == "Doyle Rowland"
...
class TestUpdateMethods:
    ...
    def on_fail_update_pof_no_data(self, error_message):
        assert error_message == (
            "do_update: No data package found for PoF " "record ID 5.2.2."
        )
        print("\033[35m\nfail_update_pof topic was broadcast")
    ...
    def test_do_update_data_manager(self, test_program_dao):
        ...
        DUT.tree.get_node("5.2.2").data["opload"].description = "Test Operating Load"
        DUT.tree.get_node("5.2.2").data["opload"].damage_model = ""
        DUT.do_update("5")
        ...
        assert DUT.tree.get_node("5.2.2").data["opload"].description == (
            "Test Operating Load"
        )
    ...
    def test_do_update_no_data_package(self, test_program_dao):
        ...
        DUT.tree.get_node("5.2.2").data.pop("opload")
    ...
    def test_do_update_wrong_data_type(self, test_program_dao):
        ...
        pub.subscribe(self.on_fail_update_pof_wrong_data_type, "fail_update_pof")

        DUT = dmPoF()
        DUT.do_connect(test_program_dao)
        DUT.do_select_all({"revision_id": 1, "hardware_id": 1})

        _opload = DUT.do_select("5.2.2", table="opload")
        ...
        pub.unsubscribe(self.on_fail_update_pof_wrong_data_type, "fail_update_pof")
