...
class TestSelectMethods:
    ...
    def on_succeed_select_all(self, tree):
        ...
        assert isinstance(
            tree.get_node(1).data['mission'], RAMSTKMission)
        assert isinstance(
            tree.get_node(2).data['mission'], RAMSTKMission)
...
class TestGetterSetter:
    ...
    def test_do_get_attributes(self, mock_program_dao):
        ...
        pub.subscribe(self.on_succeed_get_attributes,
                      'succeed_get_mission_attributes')
    ...
    def test_do_set_attributes(self, mock_program_dao):
        ...
        assert DUT.do_select(
            1, table='mission').mission_time == 12.86
...
class TestInsertMethods:
    ...
    def on_succeed_insert_sibling(self, node_id, tree):
        ...
        assert isinstance(tree, Tree)
        assert isinstance(
            tree.get_node(3).data['mission'], RAMSTKMission)
...
class TestUpdateMethods:
    ...
    def on_fail_update_non_existent_id(self, error_message):
        assert error_message == (
            'do_update: Attempted to save non-existent mission with '
            'mission ID 10.')
        print("\033[35m\nfail_update_mission topic was broadcast")

    def on_fail_update_no_data_package(self, error_message):
        assert error_message == (
            'do_update: No data package found for mission ID 1.')
        print("\033[35m\nfail_update_mission topic was broadcast")
