...
class TestRAMSTKMechanism:
    ...
    def test_ramstkmechanism_create(self, test_program_dao):
        ...
        DUT = (
            test_program_dao.session.query(RAMSTKMechanism)
            .filter(RAMSTKMechanism.mode_id == 6)
            .first()
        )
        ...
        assert DUT.description == "Test Failure Mechanism #1 for Mode ID 6"
        assert DUT.pof_include == 1
        assert DUT.rpn == 0
        assert DUT.rpn_detection == 5
        assert DUT.rpn_detection_new == 2
        assert DUT.rpn_new == 0
        assert DUT.rpn_occurrence == 7
    ...
    def test_get_attributes(self, test_program_dao):
        ...
        assert _attributes["description"] == ("Test Failure Mechanism #1 for Mode ID 6")
        assert _attributes["pof_include"] == 1
        assert _attributes["rpn"] == 0
        assert _attributes["rpn_detection"] == 5
        assert _attributes["rpn_detection_new"] == 2
        assert _attributes["rpn_new"] == 0
        assert _attributes["rpn_occurrence"] == 7
