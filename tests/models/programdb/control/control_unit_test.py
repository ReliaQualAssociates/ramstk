...
# -*- coding: utf-8 -*-
#
#       tests.models.control.control_unit_test.py is part of The RAMSTK Project
#
# All rights reserved.
...
class TestCreateModels:
    ...
    def test_record_model_create(self, test_recordmodel):
        ...
        assert isinstance(test_recordmodel, RAMSTKControlRecord)

        # Verify class attributes are properly initialized.
        assert test_recordmodel.__tablename__ == "ramstk_control"
        assert test_recordmodel.description == "Test FMEA Control #1 for Cause ID #3."
        assert test_recordmodel.type_id == "Detection"

    @pytest.mark.unit
    def test_data_manager_create(self, test_tablemodel):
        """__init__() should return a PoF data manager."""
        assert isinstance(test_tablemodel, RAMSTKControlTable)
        assert isinstance(test_tablemodel.tree, Tree)
        assert isinstance(test_tablemodel.dao, MockDAO)
        assert test_tablemodel._db_id_colname == "fld_control_id"
        assert test_tablemodel._db_tablename == "ramstk_control"
        assert test_tablemodel._tag == "control"
        ...
        assert pub.isSubscribed(
            test_tablemodel.do_get_attributes, "request_get_control_attributes"
        )
        assert pub.isSubscribed(
            test_tablemodel.do_set_attributes, "request_set_control_attributes"
        )
        assert pub.isSubscribed(
            test_tablemodel.do_set_attributes, "wvw_editing_control"
        )
        assert pub.isSubscribed(test_tablemodel.do_update, "request_update_control")
        assert pub.isSubscribed(test_tablemodel.do_get_tree, "request_get_control_tree")
        assert pub.isSubscribed(test_tablemodel.do_delete, "request_delete_control")
        assert pub.isSubscribed(test_tablemodel.do_insert, "request_insert_control")
...
class TestSelectMethods:
    ...
    def test_do_select_all(self, test_attributes, test_tablemodel):
        ...
        assert isinstance(
            test_tablemodel.tree.get_node(1).data["control"], RAMSTKControlRecord
        )
        assert isinstance(
            test_tablemodel.tree.get_node(2).data["control"], RAMSTKControlRecord
        )

    @pytest.mark.unit
    def test_do_select(self, test_attributes, test_tablemodel):
        """should return an instance of the RAMSTKControlRecord on success."""
        test_tablemodel.do_select_all(test_attributes)

        _control = test_tablemodel.do_select(1)

        assert isinstance(_control, RAMSTKControlRecord)
...
class TestInsertMethods:
    ...
    def test_do_insert_sibling(self, test_attributes, test_tablemodel):
        ...
        assert isinstance(
            test_tablemodel.tree.get_node(3).data["control"], RAMSTKControlRecord
        )
...
class TestGetterSetter:
    ...
    def test_set_record_model_attributes(self, test_attributes, test_recordmodel):
        ...
        test_attributes.pop("control_id")
    ...
    def test_set_record_model_attributes_none_value(
        ...
        test_attributes.pop("control_id")
        assert test_recordmodel.set_attributes(test_attributes) is None
        assert test_recordmodel.get_attributes()["type_id"] == ""
