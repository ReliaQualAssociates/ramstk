...
class TestRAMSTKSiteInfo():
    ...
    def test_ramstksiteinfo_create(self, test_common_dao):
        ...
        DUT = test_common_dao.session.query(RAMSTKSiteInfo).filter(
            RAMSTKSiteInfo.site_id == 3).all()[0]

        assert isinstance(DUT, RAMSTKSiteInfo)

        # Verify class attributes are properly initialized.
        assert DUT.__tablename__ == 'ramstk_site_info'
        assert DUT.site_id == 3
        assert DUT.site_name == ''
        assert DUT.product_key == ''
        ...
        assert DUT.hazard_enabled == 0
        assert DUT.stakeholder_enabled == 0
        assert DUT.allocation_enabled == 0
        assert DUT.similar_item_enabled == 0
        assert DUT.fmea_enabled == 0
        assert DUT.pof_enabled == 0
    ...
    def test_get_attributes(self, test_common_dao):
        ...
        assert _attributes['site_id'] == 3
        assert _attributes['site_name'] == ''
        assert _attributes['product_key'] == ''
        ...
        assert _attributes['hazard_enabled'] == 0
        assert _attributes['stakeholder_enabled'] == 0
        assert _attributes['allocation_enabled'] == 0
        assert _attributes['similar_item_enabled'] == 0
        assert _attributes['fmea_enabled'] == 0
        assert _attributes['pof_enabled'] == 0
