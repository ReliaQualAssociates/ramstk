...
class TestSelectRPN(SystemTestSelectMethods):
    ...
    _do_select_msg = "request_select_rpn"
...
class TestUpdateRPN:
    ...
    def test_do_update(self, integration_test_table_model):
        ...
        integration_test_table_model.tree.get_node(2).data["rpn"].value = 2
        integration_test_table_model.do_update(2)

        pub.unsubscribe(self.on_succeed_update, "succeed_update_rpn")

        assert (
            integration_test_table_model.tree.get_node(2).data["rpn"].description
            == "New RPN"
        )
        assert integration_test_table_model.tree.get_node(2).data["rpn"].value == 2

    @pytest.mark.integration
    def test_do_update_all(self, integration_test_table_model):
        """Should update all the records in the database."""
        pub.subscribe(self.on_succeed_update_all, "succeed_update_all_rpn")

        pub.sendMessage("request_update_all_rpn")

        pub.unsubscribe(self.on_succeed_update_all, "succeed_update_all_rpn")
    ...
    def test_do_update_wrong_data_type(self, integration_test_table_model):
        ...
        integration_test_table_model.tree.get_node(2).data["rpn"].value = "Hi ya"
        integration_test_table_model.do_update(2)
    ...
    def test_do_update_root_node_wrong_data_type(self, integration_test_table_model):
        ...
        integration_test_table_model.tree.get_node(2).data["rpn"].value = "Hey bud"
    ...
    def test_do_update_non_existent_id(self, integration_test_table_model):
        ...
        integration_test_table_model.do_select_all({"rpn_id": 2})
    ...
    def test_do_update_no_data_package(self, integration_test_table_model):
        ...
        integration_test_table_model.tree.get_node(2).data.pop("rpn")
        integration_test_table_model.do_update(2)
