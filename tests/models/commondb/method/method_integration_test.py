...
class TestSelectMethod(SystemTestSelectMethods):
    ...
    _do_select_msg = "request_select_method"
...
class TestUpdateMethod:
    ...
    def test_do_update(self, integration_test_table_model):
        ...
        integration_test_table_model.tree.get_node(1).data["method"].name = "New Name"
        integration_test_table_model.do_update(1)

        pub.unsubscribe(self.on_succeed_update, "succeed_update_method")

        assert (
            integration_test_table_model.tree.get_node(1).data["method"].description
            == "New Method"
        )
        assert (
            integration_test_table_model.tree.get_node(1).data["method"].name
            == "New Name"
        )
    ...
    def test_do_update_wrong_data_type(self, integration_test_table_model):
        ...
        integration_test_table_model.tree.get_node(1).data["method"].name = None
        integration_test_table_model.do_update(1)
    ...
    def test_do_update_non_existent_id(self, integration_test_table_model):
        ...
        integration_test_table_model.do_select_all({"method_id": 1})
    ...
    def test_do_update_no_data_package(self, integration_test_table_model):
        ...
        integration_test_table_model.tree.get_node(1).data.pop("method")
        integration_test_table_model.do_update(1)
...
class TestGetterSetterMethod(SystemTestGetterSetterMethods):
    ...
    _package = {"description": "Big Detector"}
    _record = RAMSTKMethodRecord
    _tag = "method"
    _test_id = 1
