...
class TestCreateControllers:
    ...
    def test_data_manager_create(self, test_datamanager):
        ...
        assert isinstance(test_datamanager, RAMSTKDesignElectricTable)
        assert isinstance(test_datamanager.tree, Tree)
        assert isinstance(test_datamanager.dao, MockDAO)
        assert test_datamanager._db_id_colname == "fld_hardware_id"
        assert test_datamanager._db_tablename == "ramstk_design_electric"
        assert test_datamanager._select_msg == "selected_revision"
        assert test_datamanager._root == 0
        assert test_datamanager._tag == "design_electric"
        assert test_datamanager._lst_id_columns == [
            "revision_id",
            "hardware_id",
        ]
        assert test_datamanager._revision_id == 0
        assert test_datamanager._record == RAMSTKDesignElectric
        assert test_datamanager.last_id == 0
        assert test_datamanager.pkey == "hardware_id"
        assert pub.isSubscribed(
            test_datamanager.do_get_attributes, "request_get_design_electric_attributes"
        )
        assert pub.isSubscribed(
            test_datamanager.do_set_attributes, "request_set_design_electric_attributes"
        )
        assert pub.isSubscribed(
            test_datamanager.do_set_attributes, "wvw_editing_design_electric"
        )
        assert pub.isSubscribed(
            test_datamanager.do_update_all, "request_update_all_design_electric"
        )
        assert pub.isSubscribed(
            test_datamanager.do_get_tree, "request_get_design_electric_tree"
        )
        assert pub.isSubscribed(test_datamanager.do_select_all, "selected_revision")
        assert pub.isSubscribed(
            test_datamanager.do_update, "request_update_design_electric"
        )
        assert pub.isSubscribed(
            test_datamanager.do_delete, "request_delete_design_electric"
        )
        assert pub.isSubscribed(
            test_datamanager.do_insert, "request_insert_design_electric"
        )
...
class TestSelectMethods:
    ...
    def test_do_select_all(self, test_attributes, test_datamanager):
        ...
        assert isinstance(
            test_datamanager.tree.get_node(1).data["design_electric"],
            RAMSTKDesignElectric,
        )
        assert isinstance(
            test_datamanager.tree.get_node(2).data["design_electric"],
            RAMSTKDesignElectric,
        )
        assert isinstance(
            test_datamanager.tree.get_node(3).data["design_electric"],
            RAMSTKDesignElectric,
        )
    ...
    def test_do_select(self, test_attributes, test_datamanager):
        ...
        _design_electric = test_datamanager.do_select(1)
...
class TestInsertMethods:
    ...
    def test_do_insert_sibling(self, test_attributes, test_datamanager):
        ...
        assert isinstance(
            test_datamanager.tree.get_node(4).data["design_electric"],
            RAMSTKDesignElectric,
        )
        assert (
            test_datamanager.tree.get_node(4).data["design_electric"].revision_id == 1
        )
        assert (
            test_datamanager.tree.get_node(4).data["design_electric"].hardware_id == 4
        )
...
class TestAnalysisMethods:
    ...
    @pytest.mark.unit
    def test_do_calculate_current_stress(self, test_attributes, test_datamanager):
        ...
        test_datamanager._do_calculate_current_ratio(1)
        _attributes = test_datamanager.do_select(1).get_attributes()

        assert _attributes["current_ratio"] == pytest.approx(0.0064)

    @pytest.mark.unit
    def test_do_calculate_power_stress(self, test_attributes, test_datamanager):
        ...
        test_datamanager._do_calculate_power_ratio(1)
        _attributes = test_datamanager.do_select(1).get_attributes()

        assert _attributes["power_ratio"] == pytest.approx(0.0009)

    @pytest.mark.unit
    def test_do_calculate_voltage_ratio(self, test_attributes, test_datamanager):
        """should calculate the power stress ratio."""
        test_datamanager.do_select_all(attributes=test_attributes)
        ...
        test_datamanager._do_calculate_voltage_ratio(1)
        _attributes = test_datamanager.do_select(1).get_attributes()
