...
class TestHardwareBoMDataController(unittest.TestCase):
    ...
    @attr(all=True, unit=True)
    def test05a_request_get_hardware_attributes(self):
        ...
        _attributes = self.DUT.request_get_attributes(1, 'general')

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_attributes['revision_id'], 1)

    @attr(all=True, unit=True)
    def test05b_request_get_design_electric_attributes(self):
        """(TestHardwareBoMDataController) request_get_attributes() should return a dict of {attribute name:attribute value} pairs for the RTKDesignElectric table."""
        self.DUT.request_select_all(1)

        _attributes = self.DUT.request_get_attributes(1, 'electrical_design')

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_attributes['hardware_id'], 1)

    @attr(all=True, unit=True)
    def test05c_request_get_design_mechanic_attributes(self):
        """(TestHardwareBoMDataController) request_get_attributes() should return a dict of {attribute name:attribute value} pairs for the RTKDesignMechanic table."""
        self.DUT.request_select_all(1)

        _attributes = self.DUT.request_get_attributes(1, 'mechanical_design')

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_attributes['hardware_id'], 1)

    @attr(all=True, unit=True)
    def test05d_request_get_mil_hdbk_f_attributes(self):
        """(TestHardwareBoMDataController) request_get_attributes() should return a dict of {attribute name:attribute value} pairs for the RTKMilHdbkF table."""
        self.DUT.request_select_all(1)

        _attributes = self.DUT.request_get_attributes(1, 'mil_hdbk_f')

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_attributes['hardware_id'], 1)

    @attr(all=True, unit=True)
    def test05e_request_get_nswc_attributes(self):
        """(TestHardwareBoMDataController) request_get_attributes() should return a dict of {attribute name:attribute value} pairs for the RTKNSWC table."""
        self.DUT.request_select_all(1)

        _attributes = self.DUT.request_get_attributes(1, 'nswc')

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_attributes['hardware_id'], 1)

    @attr(all=True, unit=True)
    def test05f_request_get_reliability_attributes(self):
        """(TestHardwareBoMDataController) request_get_attributes() should return a dict of {attribute name:attribute value} pairs for the RTKReliability table."""
        self.DUT.request_select_all(1)

        _attributes = self.DUT.request_get_attributes(1, 'reliability')

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_attributes['hardware_id'], 1)

    @attr(all=True, unit=True)
    def test06a_request_set_hardware_attributes(self):
        """(TestHardwareBoMDataController) request_set_attributes() should return a zero error code on success when setting the attributes for the RTKHardware table."""
        self.DUT.request_select_all(1)
        ...
        _error_code, _msg = self.DUT.request_set_attributes(
            1, _attributes, 'general')

        self.assertEqual(_error_code, 0)
        self.assertEqual(_msg,
                         'RTK SUCCESS: Updating RTKHardware 1 attributes.')

    @attr(all=True, unit=True)
    def test06b_request_set_design_electric_attributes(self):
        """(TestHardwareBoMDataController) request_set_attributes() should return a zero error code on success when setting the attributes for the RTKDesignElectric table."""
        self.DUT.request_select_all(1)
        ...
        _error_code, _msg = self.DUT.request_set_attributes(
            1, _attributes, 'electrical_design')

        self.assertEqual(_error_code, 0)
        self.assertEqual(
            _msg, 'RTK SUCCESS: Updating RTKDesignElectric 1 attributes.')

    @attr(all=True, unit=True)
    def test06c_request_set_design_mechanic_attributes(self):
        """(TestHardwareBoMDataController) request_set_attributes() should return a zero error code on success when setting the attributes for the RTKDesignMechanic table."""
        self.DUT.request_select_all(1)
        ...
        _error_code, _msg = self.DUT.request_set_attributes(
            1, _attributes, 'mechanical_design')

        self.assertEqual(_error_code, 0)
        self.assertEqual(
            _msg, 'RTK SUCCESS: Updating RTKDesignMechanic 1 attributes.')

    @attr(all=True, unit=True)
    def test06d_request_set_mil_hdbk_f_attributes(self):
        """(TestHardwareBoMDataController) request_set_attributes() should return a zero error code on success when setting the attributes for the RTKMilHdbkF table."""
        self.DUT.request_select_all(1)
        ...
        _error_code, _msg = self.DUT.request_set_attributes(
            1, _attributes, 'mil_hdbk_f')

        self.assertEqual(_error_code, 0)
        self.assertEqual(_msg,
                         'RTK SUCCESS: Updating RTKMilHdbkF 1 attributes.')

    @attr(all=True, unit=True)
    def test06e_request_set_nswc_attributes(self):
        """(TestHardwareBoMDataController) request_set_attributes() should return a zero error code on success when setting the attributes for the RTKNSWC table."""
        self.DUT.request_select_all(1)
        ...
        self.assertEqual(_error_code, 0)
        self.assertEqual(_msg, 'RTK SUCCESS: Updating RTKNSWC 1 attributes.')

    @attr(all=True, unit=True)
    def test06f_request_set_reliability_attributes(self):
        """(TestHardwareBoMDataController) request_set_attributes() should return a zero error code on success when setting the attributes for the RTKReliability table."""
        self.DUT.request_select_all(1)
        ...
        _error_code, _msg = self.DUT.request_set_attributes(
            1, _attributes, 'reliability')

        self.assertEqual(_error_code, 0)
        self.assertEqual(_msg,
                         'RTK SUCCESS: Updating RTKReliability 1 attributes.')
