# -*- coding: utf-8 -*-
#
#       tests.dao.commondb.test_rtkuser.py is part of The RTK Project
#
# All rights reserved.
...
def test_rtkuser_create(test_common_dao):
    """ __init__() should create an RTKUser model. """
    _session = test_common_dao.RTK_SESSION(
        bind=test_common_dao.engine, autoflush=False, expire_on_commit=False)
    DUT = _session.query(RTKUser).first()

    assert isinstance(DUT, RTKUser)
...
def test_set_attributes(test_common_dao):
    ...
    assert _msg == ("RTK SUCCESS: Updating RTKUser {0:d} "
...
def test_set_attributes_missing_key(test_common_dao):
    ...
    assert _msg == ("RTK ERROR: Missing attribute 'user_lname' in attribute "
                    "dictionary passed to RTKUser.set_attributes().")
