...
def test_set_attributes(test_dao):
    ...
    assert _msg == ("RAMSTK SUCCESS: Updating RAMSTKAllocation 1 attributes.")
...
def test_set_attributes_too_few_passed(test_dao):
    ...
    assert _msg == ("RAMSTK ERROR: Missing attribute 'env_factor' in attribute "
                    "dictionary passed to RAMSTKAllocation.set_attributes().")
...
def test_equal_apportionment(test_dao):
    ...
    _session = test_dao.RAMSTK_SESSION(
        bind=test_dao.engine, autoflush=False, expire_on_commit=False)
    DUT = _session.query(RAMSTKAllocation).filter(
        RAMSTKAllocation.hardware_id == 2).all()
