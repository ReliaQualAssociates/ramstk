...
def test_ramstkvalidation_create(test_dao):
    ...
    DUT = _session.query(RAMSTKValidation).first()
...
def test_set_attributes(test_dao):
    ...
    assert _msg == ("RAMSTK SUCCESS: Updating RAMSTKValidation {0:d} "
...
def test_set_attributes_missing_key(test_dao):
    ...
    assert _msg == ("RAMSTK ERROR: Missing attribute 'status' in attribute "
                    "dictionary passed to RAMSTKValidation.set_attributes().")
