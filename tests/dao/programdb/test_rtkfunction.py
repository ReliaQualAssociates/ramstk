# -*- coding: utf-8 -*-
#
#       tests.dao.programdb.test_rtkfunction.py is part of The RTK Project
#
# All rights reserved.
...
from rtk.dao.programdb.RTKFunction import RTKFunction
...
def test_rtkfunction_create(test_dao):
    ...
    DUT = _session.query(RTKFunction).first()

    assert isinstance(DUT, RTKFunction)
...
def test_set_attributes(test_dao):
    ...
    assert _msg == ("RTK SUCCESS: Updating RTKFunction {0:d} "
...
def test_set_attributes_too_few_passed(test_dao):
    ...
    assert _msg == ("RTK ERROR: Missing attribute 'type_id' in attribute "
                    "dictionary passed to RTKFunction.set_attributes().")
