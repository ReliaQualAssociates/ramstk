...
from analyses.prediction import Crystal
...
class TestCrystalModule(unittest.TestCase):
    """Class for testing the Crystal module functions."""
    ...
    def setUp(self):
        """Set up the test fixture for the Crystal module."""
        self._lst_lambda_b = [
            0.032, 0.096, 0.32, 0.19, 0.51, 0.38, 0.54, 0.70, 0.90, 0.74,
            0.016, 0.42, 1.0, 16.0
        ]
        self._lst_piQ = [1.0, 2.1]
    ...
    def test00a_calculate_mil_hdbk_217f_part_count(self):
        ...
        _attributes, _msg = Crystal.calculate_217f_part_count(
            **self._attributes)

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_msg, '')
        self.assertEqual(_attributes['lambda_b'], 0.032)
        self.assertEqual(_attributes['piQ'], 2.1)
        self.assertAlmostEqual(_attributes['hazard_rate_active'], 0.0672)

    @attr(all=True, unit=True)
    def test00b_calculate_mil_hdbk_217f_part_count_all(self):
        """(TestCrystalModule) calculate_mil_hdbk_217f_part_count() should return a dictionary of updated values on success for each active environment and quality level combination for crystals."""
        self._attributes['add_adj_factor'] = 0.0
        self._attributes['mult_adj_factor'] = 1.0
        self._attributes['duty_cycle'] = 100.0
        self._attributes['quantity'] = 1
        ...
        for environment_active_id in xrange(1, 14):
            ...
            for quality_id in xrange(1, 3):
                ...
                _test_msg = 'test00b_calculate_mil_hdbk_217f_part_count_mechanical_1_{}_{} ' \
                            'failed'.format(environment_active_id, quality_id)
    ...
    def test00c_calculate_mil_hdbk_217f_part_count_missing_environment(self):
        ...
        self.assertEqual(
            _msg,
            'RTK WARNING: Base hazard rate is 0.0 when calculating crystal, '
    ...
    def test01a_calculate_mil_hdbk_217f_part_stress(self):
        ...
        self._attributes['frequency_operating'] = 10.0
        self._attributes['quality_id'] = 1
        ...
        _attributes, _msg = Crystal.calculate_217f_part_stress(
            **self._attributes)

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_msg, '')
        self.assertAlmostEqual(_attributes['lambda_b'], 0.02207717)
        self.assertEqual(_attributes['piQ'], 1.0)
        self.assertEqual(_attributes['piE'], 6.0)
        self.assertAlmostEqual(_attributes['hazard_rate_active'], 0.1324630)

    @attr(all=True, unit=True)
    def test02a_calculate(self):
        """(TestCrystalModule) calculate() should return should return a dictionary of updated values on success."""
        self._attributes['environment_active_id'] = 1
        self._attributes['quality_id'] = 2
        self._attributes['add_adj_factor'] = 0.0
        self._attributes['mult_adj_factor'] = 1.0
        ...
        _attributes, _msg = Crystal.calculate(**self._attributes)
