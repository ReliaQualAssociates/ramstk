...
from analyses.prediction import Meter
...
class TestMeterModule(unittest.TestCase):
    """Class for testing the Meter module functions."""
    ...
    def setUp(self):
        """Set up the test fixture for the Meter module."""
        self._dic_lambda_b = {
            1: [[
            ...
            2: [[
                10.0, 20.0, 120.0, 70.0, 180.0, 50.0, 80.0, 160.0, 250.0,
                260.0, 5.0, 140.0, 380.0, 0.0
            ], [
                15.0, 30.0, 180.0, 105.0, 270.0, 75.0, 120.0, 240.0, 375.0,
                390.0, 7.5, 210.0, 570.0, 0.0
            ], [
                40.0, 80.0, 480.0, 280.0, 720.0, 200.0, 320.0, 640.0, 1000.0,
                1040.0, 20.0, 560.0, 1520.0, 0.0
            ]]
        }
        self._dic_piQ = {1: [1.0, 3.4]}

    @attr(all=True, unit=True)
    def test00a_calculate_mil_hdbk_217f_part_count(self):
        """(TestMeterModule) calculate_mil_hdbk_217f_part_count() should return a dictionary of updated values on success."""
        ...
        _attributes, _msg = Meter.calculate_217f_part_count(**self._attributes)

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_msg, '')
        self.assertEqual(_attributes['lambda_b'], 0.15)
        self.assertEqual(_attributes['piQ'], 3.4)
        self.assertAlmostEqual(_attributes['hazard_rate_active'], 0.51)

    @attr(all=True, unit=True)
    def test00b_calculate_mil_hdbk_217f_part_count_all_panel(self):
        """(TestMeterModule) calculate_mil_hdbk_217f_part_count() should return a dictionary of updated values on success for each active environment and quality level combination for panel meters."""
        self._attributes['add_adj_factor'] = 0.0
        self._attributes['mult_adj_factor'] = 1.0
        self._attributes['duty_cycle'] = 100.0
        self._attributes['quantity'] = 1
        ...
        self._attributes['subcategory_id'] = 1
        for environment_active_id in xrange(1, 12):
            ...
            for quality_id in xrange(1, 3):
                ...
                for type_id in xrange(1, 3):
                    ...
                    _attributes, _msg = Meter.calculate_217f_part_count(
                        **self._attributes)
    ...
    @attr(all=True, unit=True)
    def test00c_calculate_mil_hdbk_217f_part_count_all_elapsed_time(self):
        """(TestMeterModule) calculate_mil_hdbk_217f_part_count() should return a dictionary of updated values on success for each active environment and quality level combination for elapsed time meters."""
        self._attributes['add_adj_factor'] = 0.0
        self._attributes['mult_adj_factor'] = 1.0
        self._attributes['duty_cycle'] = 100.0
        self._attributes['quantity'] = 1
        ...
        for environment_active_id in xrange(1, 13):
            self._attributes['environment_active_id'] = environment_active_id
            for type_id in xrange(1, 3):
                self._attributes['type_id'] = type_id
                _test_msg = 'test00b_calculate_mil_hdbk_217f_part_count_elapsed_time_2_{}_{} ' \
                ...
                self.assertEqual(_attributes['piQ'], 1.0, _test_msg)
                self.assertEqual(_attributes['hazard_rate_active'],
                                 _lambda_b * _attributes['piQ'], _test_msg)

    @attr(all=True, unit=True)
    def test00c_calculate_mil_hdbk_217f_part_count_missing_subcategory(self):
        """(TestMeterModule) calculate_mil_hdbk_217f_part_count() should return an error message when the subcategory ID is missing."""
        ...
        self.assertEqual(
            _msg,
            'RTK WARNING: Base hazard rate is 0.0 when calculating meter, '
    ...
    def test00d_calculate_mil_hdbk_217f_part_count_missing_environment(self):
        """(TestMeterModule) calculate_mil_hdbk_217f_part_count() should return an error message when the active environment ID is missing."""
        self._attributes['subcategory_id'] = 1
    ...
    def test01a_calculate_mil_hdbk_217f_part_stress(self):
        """(TestMeterModule) calculate_mil_hdbk_217f_part_stress() should return a dictionary of updated values on success."""
        self._attributes['subcategory_id'] = 1
        self._attributes['type_id'] = 1
        self._attributes['application_id'] = 2
        ...
        _attributes, _msg = Meter.calculate_217f_part_stress(
            **self._attributes)

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_msg, '')
        self.assertEqual(_attributes['lambda_b'], 0.09)
        self.assertEqual(_attributes['piQ'], 1.0)
        self.assertEqual(_attributes['piE'], 12.0)
        self.assertEqual(_attributes['piA'], 1.7)
        self.assertEqual(_attributes['piF'], 1.0)
        self.assertAlmostEqual(_attributes['hazard_rate_active'], 1.836)

    @attr(all=True, unit=True)
    def test02a_calculate(self):
        """(TestMeterModule) calculate() should return should return a dictionary of updated values on success."""
        self._attributes['subcategory_id'] = 1
        self._attributes['type_id'] = 2
        self._attributes['environment_active_id'] = 1
        self._attributes['quality_id'] = 2
        ...
        _attributes, _msg = Meter.calculate(**self._attributes)
