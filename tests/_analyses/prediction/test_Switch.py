...
from analyses.prediction import Switch
...
class TestSwitchModule(unittest.TestCase):
    """Class for testing the Switch module functions."""
    ...
    def setUp(self):
        """Set up the test fixture for the Switch module."""
        self._dic_lambda_b = {
            1: [
                0.0010, 0.0030, 0.018, 0.0080, 0.029, 0.010, 0.018, 0.013,
                0.022, 0.046, 0.0005, 0.025, 0.067, 1.2
            ],
            2: [
                0.15, 0.44, 2.7, 1.2, 4.3, 1.5, 2.7, 1.9, 3.3, 6.8, 0.74, 3.7,
                9.9, 180.0
            ],
            3: [
            ...
            4: [
                0.56, 1.7, 10.0, 4.5, 16.0, 5.6, 10.0, 7.3, 12.0, 26.0, 0.26,
                14.0, 38.0, 670.0
            ],
            5: [
    ...
    def test00a_calculate_mil_hdbk_217f_part_count(self):
        """(TestSwitchModule) calculate_mil_hdbk_217f_part_count() should return a dictionary of updated values on success."""
        ...
        _attributes, _msg = Switch.calculate_217f_part_count(
            **self._attributes)

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_msg, '')
        self.assertEqual(_attributes['lambda_b'], 0.15)
        self.assertEqual(_attributes['piQ'], 20.0)
        self.assertAlmostEqual(_attributes['hazard_rate_active'], 3.0)

    @attr(all=True, unit=True)
    def test00b_calculate_mil_hdbk_217f_part_count_all(self):
        """(TestSwitchModule) calculate_mil_hdbk_217f_part_count() should return a dictionary of updated values on success for each subcategory, active environment, and quality level combination."""
        self._attributes['add_adj_factor'] = 0.0
        self._attributes['mult_adj_factor'] = 1.0
        self._attributes['duty_cycle'] = 100.0
        self._attributes['quantity'] = 1
        ...
        for subcategory_id in xrange(1, 3):
            ...
            for environment_active_id in xrange(1, 15):
                ...
                for quality_id in xrange(1, 3):
                    ...
                    _piQ = self._dic_piQ[subcategory_id][quality_id - 1]
    ...
    def test00c_calculate_mil_hdbk_217f_part_count_missing_subcategory(self):
        """(TestSwitchModule) calculate_mil_hdbk_217f_part_count() should return an error message when the subcategory ID is missing."""
        ...
        self.assertEqual(
            _msg,
            'RTK WARNING: Base hazard rate is 0.0 when calculating switch, ' \
            'hardware ID: 6, subcategory ID: 0, active environment ID: 1' \
            'RTK WARNING: piQ is 0.0 when calculating switch, hardware ID: ' \
            '6 and quality ID: 1'
        )
    ...
    def test00d_calculate_mil_hdbk_217f_part_count_missing_environment(self):
        """(TestSwitchModule) calculate_mil_hdbk_217f_part_count() should return an error message when the active environment ID is missing."""
    ...
    def test01a_calculate_mil_hdbk_217f_part_stress(self):
        """(TestSwitchModule) calculate_mil_hdbk_217f_part_stress() should return a dictionary of updated values on success."""
        ...
        _attributes, _msg = Switch.calculate_217f_part_stress(
            **self._attributes)

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_msg, '')
        self.assertAlmostEqual(_attributes['current_ratio'], 0.3)
        self.assertAlmostEqual(_attributes['lambda_b'], 0.00682)
        self.assertEqual(_attributes['piQ'], 10.0)
        self.assertEqual(_attributes['piE'], 8.0)
        self.assertAlmostEqual(_attributes['piCYC'], 5.0)
        self.assertAlmostEqual(_attributes['piL'], 1.1509929)
        self.assertAlmostEqual(_attributes['piC'], 0.0)
        self.assertAlmostEqual(_attributes['hazard_rate_active'], 0.3139909)

    @attr(all=True, unit=True)
    def test02a_calculate_dormant_hazard_rate(self):
        """(TestSwitchModule) calculate_dormant_hazard_rate() should return a dictionary of updated values on success."""
        self._attributes['environment_active_id'] = 4
        self._attributes['environment_dormant_id'] = 3
        self._attributes['hazard_rate_active'] = 1.005887691

        _attributes, _msg = Switch.calculate_dormant_hazard_rate(
            **self._attributes)

        self.assertTrue(isinstance(_attributes, dict))
        self.assertEqual(_msg, '')
        self.assertAlmostEqual(_attributes['hazard_rate_dormant'], 0.4023551)

    @attr(all=True, unit=True)
    def test02b_calculate_dormant_hazard_rate_unknown_env_id(self):
        """(TestSwitchModule) calculate_dormant_hazard_rate() should return an error message when an unknown environment ID is passed."""
    ...
    def test03a_overstressed_mild_env(self):
        """(TestSwitchModule) overstressed() should return overstress=False and reason='' on success without an overstressed condition in non-harsh environment."""
        self._attributes['environment_active_id'] = 1
        self._attributes['current_operating'] = 0.18
        self._attributes['current_rated'] = 0.5

        _attributes = Switch.overstressed(**self._attributes)
    ...
    def test03b_overstressed_mild_env_current_margin(self):
        """(TestSwitchModule) overstressed() should return overstress=True on success with operating current too close to rated in a non-harsh environment."""
        self._attributes['environment_active_id'] = 1
    ...
    def test03c_overstressed_harsh_env(self):
        """(TestSwitchModule) overstressed() should return overstress=False and reason='' on success without an overstressed condition in a harsh environment."""
    ...
    def test03d_overstressed_harsh_env_current_margin(self):
        """(TestSwitchModule) overstressed() should return overstress=True on success with operating current too close to rated in a harsh environment."""
        self._attributes['environment_active_id'] = 3
    ...
    def test04a_calculate(self):
        """(TestSwitchModule) calculate() should return should return a dictionary of updated values on success."""
        self._attributes['subcategory_id'] = 2
        self._attributes['environment_active_id'] = 1
        ...
        _attributes, _msg = Switch.calculate(**self._attributes)
        ...
        self.assertEqual(_msg, '')
        self.assertAlmostEqual(_attributes['current_ratio'], 0.3)
        self.assertAlmostEqual(_attributes['lambda_b'], 0.1036)
        self.assertEqual(_attributes['piQ'], 10.0)
        self.assertEqual(_attributes['piE'], 8.0)
        self.assertAlmostEqual(_attributes['piCYC'], 5.0)
        self.assertAlmostEqual(_attributes['piL'], 1.1509929)
        self.assertAlmostEqual(_attributes['piC'], 0.0)
        self.assertAlmostEqual(_attributes['hazard_rate_active'], 4.7697148)
