...
# -*- coding: utf-8 -*-
#
#       tests.analyses.functions.mil_hdbk_217f.test_capacitor.py is part of The
#       RAMSTK Project
#
# All rights reserved.
...
@pytest.mark.parametrize("specification_id", [1, 2, None])
def test_get_part_count_lambda_b_list(subcategory_id, specification_id):
    """get_part_count_lambda_b_list() should return a list of base hazard rates on success or raise a KeyError when no key exists."""
    if subcategory_id == 1 and specification_id is not None:
        _test_lst = Capacitor.PART_COUNT_LAMBDA_B[subcategory_id][
            specification_id
        ]
    else:
        _test_lst = Capacitor.PART_COUNT_LAMBDA_B[subcategory_id]

    if subcategory_id == 1 and specification_id is None:
        with pytest.raises(KeyError):
            _lst_lambda_b = Capacitor.get_part_count_lambda_b_list(
                subcategory_id=subcategory_id,
                specification_id=specification_id,
            )
    else:
        _lst_lambda_b = Capacitor.get_part_count_lambda_b_list(
            subcategory_id=subcategory_id, specification_id=specification_id,
        )
        assert isinstance(_lst_lambda_b, list)
        assert _lst_lambda_b == _test_lst
...
@pytest.mark.parametrize(
    "ref_temp", [65.0, 70.0, 85.0, 105.0, 125.0, 150.0, 170.0, 175.0, 200.0],
)
def test_calculate_part_stress_lambda_b(subcategory_id, ref_temp):
    ...
    _base_hr = Capacitor.calculate_part_stress_lambda_b(
        subcategory_id, ref_temp, 45.0, 0.65,
    )
    ...
    if subcategory_id == 1 and ref_temp == 105.0:
        assert pytest.approx(_base_hr, 0.12880427)
    if subcategory_id == 10 and ref_temp == 175.0:
        assert pytest.approx(_base_hr, 0.06621194)
    if subcategory_id == 15 and ref_temp == 65.0:
        assert pytest.approx(_base_hr, 0.0068154785)
    if subcategory_id == 19 and ref_temp == 200.0:
        assert pytest.approx(_base_hr, 0.65589138)
...
def test_calculate_capacitance_factor(subcategory_id):
    ...
    assert pytest.approx(_pi_cv, _results[subcategory_id])
...
def test_calculate_series_resistance_factor(resistance):
    ...
    _pi_sr, _error_msg = Capacitor.calculate_series_resistance_factor(
        resistance, 10.0, 0.1,
    )
...
def test_calculate_series_resistance_factor_zero_voltage():
    """calculate_series_resistance_factor() should return piSR=0.33 and a non-empty error message when passed zreo voltage."""
    _pi_sr, _error_msg = Capacitor.calculate_series_resistance_factor(
        1.0, 0.0, 0.0,
    )
    ...
    assert isinstance(_pi_sr, float)
    assert _error_msg == (
        "RAMTK WARNING: Calculated series resistance factor (piSR) may be "
        "erroneous due to bad inputs.  ESR: 1.000000, DC operating voltage: "
        "0.000000, ac operating voltage: 0.000000 "
    )
    assert _pi_sr == 0.33
...
def test_calculate_part_stress(subcategory_id):
    ...
    if subcategory_id == 1:
        assert pytest.approx(_attributes['lambda_b'], 0.029446888)
        assert pytest.approx(_attributes['piCV'], 0.36177626)
        assert pytest.approx(_attributes['hazard_rate_active'], 0.010653185)
    elif subcategory_id == 12:
        assert pytest.approx(_attributes['lambda_b'] == 0.022463773)
        assert pytest.approx(_attributes['piCV'], 0.2198982)
        assert _attributes['piSR'] == 0.33
        assert pytest.approx(_attributes['hazard_rate_active'], 0.0016301152)
    elif subcategory_id == 13:
        assert pytest.approx(_attributes['lambda_b'], 0.0098840599)
        assert _attributes['piC'] == 0.3
        assert pytest.approx(_attributes['piCV'], 0.3564805)
        assert pytest.approx(_attributes['hazard_rate_active'], 0.0010570424)
    elif subcategory_id == 19:
        assert pytest.approx(_attributes['lambda_b'], 0.48854794)
        assert _attributes['piCF'] == 0.1
        assert _attributes['piCV'] == 1.0
        assert pytest.approx(_attributes['hazard_rate_active'], 0.048854794)
