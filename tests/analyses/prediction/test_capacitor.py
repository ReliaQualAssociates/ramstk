...
from ramstk.analyses import data
from ramstk.analyses.prediction import Capacitor, Component
...
def test_voltage_overstress_harsh_environment(
    ...
    _attributes = Component.do_check_overstress(data.RAMSTK_STRESS_LIMITS, **_attributes)
    ...
    if voltage_rated == 20.0:
        ...
        assert _attributes['reason'] == (
            '1. Operating voltage > 60.0% rated '
...
def test_temperature_overstress_harsh_environment(
    ...
    if temperature_active == 48.7:
        ...
        assert _attributes['reason'] == (
            ...
            'temperature in harsh environment.\n'
        )
...
def test_check_variable_zero():
    ...
    _msg = Capacitor.do_check_variables(ATTRIBUTES)
    assert _msg == (
        'RAMSTK WARNING: Base hazard rate is 0.0 when ' \
