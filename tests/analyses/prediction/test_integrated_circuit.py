...
from ramstk.analyses.prediction import IntegratedCircuit, Component
...
def test_calculate_mil_hdbk_217f_part_count(technology_id, quality_id,
    ...
    try:
        lambda_b = PART_COUNT_LAMBDA_B[ATTRIBUTES['subcategory_id']][
            ATTRIBUTES['technology_id']][1][environment_active_id - 1]
...
def test_under_voltage(voltage_rated, environment_active_id):
    ...
    if voltage_rated == 3.3:
        ...
        assert _attributes['reason'] == ('1. Operating voltage < 95% rated '
                                         'voltage.\n')
