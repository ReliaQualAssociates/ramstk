# -*- coding: utf-8 -*-
#
#       tests.dao.programmdb.test_rtkhardware.py is part of The RTK Project
#
# All rights reserved.
...
def test_rtkhardware_create(test_dao):
    """ __init__() should create an RTKHardware model. """
    _session = test_dao.RTK_SESSION(
        bind=test_dao.engine, autoflush=False, expire_on_commit=False)
    DUT = _session.query(RTKHardware).first()

    assert isinstance(DUT, RTKHardware)
...
def test_set_attributes(test_dao):
    ...
    assert _msg == ("RTK SUCCESS: Updating RTKHardware {0:d} "
...
def test_set_attributes_missing_key(test_dao):
    ...
    assert _msg == ("RTK ERROR: Missing attribute 'name' in attribute "
                    "dictionary passed to RTKHardware.set_attributes().")
