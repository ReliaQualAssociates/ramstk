...
from ramstk.db.common import (
    _do_load_action_variables,
    _do_load_hardware_variables,
    _do_load_incident_variables,
    _do_load_miscellaneous_variables,
    _do_load_pof_variables,
    _do_load_rpn_variables,
    _do_load_severity,
    _do_load_user_workgroups,
    _load_fmea_tables,
    _load_hazard_analysis_tables,
    _load_incident_report_tables,
    _load_miscellaneous_tables,
    _load_pof_tables,
    _load_site_info,
    do_add_administrator,
    do_create_common_db,
    do_load_variables,
...
def test_do_create_common_db(monkeypatch):
    ...
    _record = (
        TEST_COMMON_DB.session.query(RAMSTKUser).filter(RAMSTKUser.user_id == 1).first()
    )
...
class TestLoadCommonTables:
    ...
    def test_do_load_action_variables(
        ...
        assert test_toml_user_configuration.RAMSTK_ACTION_STATUS == {
            ...
            14: ("Ready for Closure", "Action is ready to be closed.", "action"),
    ...
    def test_do_load_hardware_variables(
        ...
        assert test_toml_user_configuration.RAMSTK_FAILURE_MODES == {
            ...
            3: {
                24: {3: ["Parameter Change", 0.2, "FMD-97"]},
                25: {},
                26: {},
                27: {},
                28: {},
        ...
        assert test_toml_user_configuration.RAMSTK_SUBCATEGORIES == {
            ...
            4: {
                ...
                49: "Fixed, Ceramic, Temperature Compensating and Chip (CC, CCR, CDR)",
            ...
            5: {58: "Transformer", 59: "Coil"},
            6: {60: "Mechanical", 61: "Solid State"},
            7: {
    ...
    def test_do_load_incident_variables(
        ...
        assert test_toml_user_configuration.RAMSTK_INCIDENT_STATUS == {
            ...
            9: ("Ready for Closure", "Incident is ready to be closed.", "incident"),
    ...
    def test_do_load_rpn_variables(self, test_common_dao, test_toml_user_configuration):
        ...
        assert test_toml_user_configuration.RAMSTK_RPN_DETECTION == {
            1: {
                "rpn_id": 21,
                "name": "Almost Certain",
                "description": "Design control will almost certainly detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 1,
            },
            2: {
                "rpn_id": 22,
                "name": "Very High",
                "description": "Very high chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 2,
            },
            3: {
                "rpn_id": 23,
                "name": "High",
                "description": "High chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 3,
            },
            4: {
                ...
                "description": "Moderately high chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 4,
            },
            5: {
                "rpn_id": 25,
                "name": "Moderate",
                "description": "Moderate chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 5,
            },
            6: {
                "rpn_id": 26,
                "name": "Low",
                "description": "Low chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 6,
            },
            7: {
                "rpn_id": 27,
                "name": "Very Low",
                "description": "Very low chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 7,
            },
            8: {
                "rpn_id": 28,
                "name": "Remote",
                "description": "Remote chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 8,
            },
            9: {
                "rpn_id": 29,
                "name": "Very Remote",
                "description": "Very remote chance the existing design controls will or can detect a potential mechanism/cause and subsequent failure mode.",
                "rpn_type": "detection",
                "value": 9,
            },
            10: {
                ...
                "description": "Existing design controls will not or cannot detect a potential mechanism/cause and subsequent failure mode; there is no design control.",
                "rpn_type": "detection",
                "value": 10,
            },
        }
        ...
        assert test_toml_user_configuration.RAMSTK_RPN_SEVERITY == {
            ...
            3: {
                "rpn_id": 3,
                "name": "Minor",
                "description": "System operable with some degradation of performance.",
                "rpn_type": "severity",
                "value": 3,
            },
            4: {
                "rpn_id": 4,
                "name": "Very Low",
                "description": "System operable with significant degradation of performance.",
                "rpn_type": "severity",
                "value": 4,
            },
            5: {
            ...
            8: {
                "rpn_id": 8,
                "name": "Very High",
                "description": "System inoperable with destructive failure without compromising safety.",
                "rpn_type": "severity",
                "value": 8,
            },
            9: {
                ...
                "description": "Failure effects safe system operation with warning.",
                "rpn_type": "severity",
                "value": 9,
            },
            10: {
                ...
                "description": "Failure effects safe system operation without warning.",
                "rpn_type": "severity",
                "value": 10,
            },
        }
    ...
    def test_do_load_variables(self, test_common_dao, test_toml_user_configuration):
        """_do_load_variables() should return None."""
        assert do_load_variables(test_common_dao, test_toml_user_configuration) is None
