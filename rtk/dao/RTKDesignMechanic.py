...
class RTKDesignMechanic(RTK_BASE):
    ...
    def get_attributes(self):
        ...
        _attributes = (
            self.hardware_id, self.altitude_operating, self.application_id,
            self.balance_id, self.clearance, self.casing_id,
            self.contact_pressure, self.deflection, self.diameter_coil,
            self.diameter_inner, self.diameter_outer, self.diameter_wire,
            self.filter_size, self.flow_design, self.flow_operating,
            self.frequency_operating, self.friction, self.impact_id,
            self.leakage_allowable, self.length, self.length_compressed,
            self.length_relaxed, self.load_design, self.load_id,
            self.load_operating, self.lubrication_id, self.manufacturing_id,
            self.material_id, self.meyer_hardness, self.misalignment_angle,
            self.n_ten, self.n_cycles, self.n_elements, self.offset,
            self.particle_size, self.pressure_contact, self.pressure_delta,
            self.pressure_downstream, self.pressure_rated,
            self.pressure_upstream, self.rpm_design, self.rpm_operating,
            self.service_id, self.spring_index, self.surface_finish,
            self.technology_id, self.thickness, self.torque_id, self.type_id,
            self.viscosity_design, self.viscosity_dynamic, self.water_per_cent,
            self.width_minimum)
    ...
    def set_attributes(self, attributes):
        ...
        try:
            ...
            self.misalignment_angle = float(
                none_to_default(attributes[28], 0.0))
            ...
            self.pressure_downstream = float(
                none_to_default(attributes[36], 0.0))
            ...
            self.viscosity_dynamic = float(
                none_to_default(attributes[49], 0.0))
