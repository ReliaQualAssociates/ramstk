...
groups = {0: ('Engineering, Design', 'workgroup'),
...
models = {0: ('Equal Apportionment', 'allocation'),
...
types = {0: ('CALC', 'Calculated', 'cost'),
         ...
         39: ('EMA', 'System Engineering, Electromagnetic Analysis',
              'validation'),
         ...
         44: ('SCA', 'System Engineering, Sneak Circuit Analysis',
              'validation'),
...
categories = {0: ('IC', 'Integrated Circuit', 'hardware', 1),
              ...
              15: ('MAJ', 'Major', 'risk', 6),
              16: ('Batch (General)',
                   'Can be run as a normal batch job and makes no ' \
                   'unusual hardware or input-output actions ' \
                   '(e.g., payroll program and wind tunnel data ' \
                   'analysis program). Small, throwaway programs ' \
                   'for preliminary analysis also fit in this ' \
                   'category.',
                   'software', 1),
              17: ('Event Control',
...
manufacturers = {0: ('Sprague', 'New Hampshire', '13606'),
                      1: ('Xilinx', '', ''),
                      2: ('National Semiconductor', 'California', '27014')}

units = {0: ('lbf', 'Pounds Force', 'measurement'),
...
criticalitys = {0: ('Catastrophic',
                         'Could result in death, permanent total ' \
                         'disability, loss exceeding $1M, or ' \
                         'irreversible severe environmental damage ' \
                         'that violates law or regulation.', 'I', 4),
                     1: ('Critical',
                         'Could result in permanent partial disability, ' \
                         'injuries or occupational illness that may ' \
                         'result in hospitalization of at least three ' \
                         'personnel, loss exceeding $200K but less ' \
                         'than $1M, or reversible environmental damage ' \
                         'causing a violation of law or regulation.',
                         'II', 3),
                     2: ('Marginal',
                         'Could result in injury or occupational ' \
                         'illness resulting in one or more lost work ' \
                         'days(s), loss exceeding $10K but less than ' \
                         '$200K, or mitigatible environmental damage ' \
                         'without violation of law or regulation where ' \
                         'restoration activities can be accomplished.',
                         'III', 2),
                     3: ('Negligble',
                         'Could result in injury or illness not ' \
                         'resulting in a lost work day, loss exceeding ' \
                         '$2K but less than $10K, or minimal ' \
                         'environmental damage not violating law or ' \
                         'regulation.', 'IV', 1)}

rpns = {0 : ('None', 'No effect.', 'severity', 1),
             1 : ('Very Minor',
                  'System operable with minimal interference.', 'severity',
                  2),
             2 : ('Minor',
             ...
             4 : ('Low', 'System inoperable without damage.', 'severity',
                  5),
             ...
             8 : ('Hazardous, with warning',
                  ...
                  'severity', 9),
             9 : ('Hazardous, without warning',
                  'Failure effects safe system operation without ' \
                  'warning.', 'severity', 10),
             10: ('Remote', 'Failure rate is 1 in 1,500,000.',
                  'occurrence', 1),
             11: ('Very Low', 'Failure rate is 1 in 150,000.',
                  'occurrence', 2),
             12: ('Low', 'Failure rate is 1 in 15,000', 'occurrence', 3),
             13: ('Moderately Low', 'Failure rate is 1 in 2000.',
                  'occurrence', 4),
             14: ('Moderate', 'Failure rate is 1 in 400.', 'occurrence',
                  5),
             15: ('Moderately High', 'Failure rate is 1 in 80.',
                  'occurrence', 6),
             16: ('High', 'Failure rate is 1 in 20.', 'occurrence', 7),
             17: ('Very High', 'Failure rate is 1 in 8.', 'occurrence', 8),
             18: ('Extremely High', 'Failure rate is 1 in 3.',
                  'occurrence', 9),
             19: ('Dangerously High', 'Failure rate is > 1 in 2.',
                  'occurrence', 10),
             20: ('Almost Certain',
                  'Design control will almost certainly detect a ' \
                  'potential mechanism/cause and subsequent failure mode.',
                  'detection', 1),
             21: ('Very High',
                  'Very high chance the existing design controls will ' \
                  'or can detect a potential mechanism/cause and ' \
                  'subsequent failure mode.', 'detection', 2),
             22: ('High',
                  'High chance the existing design controls will or ' \
                  'can detect a potential mechanism/cause and ' \
                  'subsequent failure mode.', 'detection', 3),
             23: ('Moderately High',
                  'Moderately high chance the existing design controls ' \
                  'will or can detect a potential mechanism/cause and ' \
                  'subsequent failure mode.', 'detection', 4),
             24: ('Moderate',
                  'Moderate chance the existing design controls will ' \
                  'or can detect a potential mechanism/cause and ' \
                  'subsequent failure mode.', 'detection', 5),
             25: ('Low',
                  'Low chance the existing design controls will or can ' \
                  'detect a potential mechanism/cause and subsequent ' \
                  'failure mode.', 'detection', 6),
             26: ('Very Low',
                  'Very low chance the existing design controls will ' \
                  'or can detect a potential mechanism/cause and ' \
                  'subsequent failure mode.', 'detection', 7),
             27: ('Remote',
                  'Remote chance the existing design controls will or ' \
                  'can detect a potential mechanism/cause and ' \
                  'subsequent failure mode.', 'detection', 8),
             28: ('Very Remote',
                  'Very remote chance the existing design controls ' \
                  'will or can detect a potential mechanism/cause and ' \
                  'subsequent failure mode.', 'detection', 9),
             29: ('Absolute Uncertainty',
                  'Existing design controls will not or cannot detect ' \
                  'a potential mechanism/cause and subsequent failure ' \
                  'mode; there is no design control.', 'detection', 10)}

levels = {0: ('Software System', 'software', 0),
...
applications = {0: ('Airborne', 0.0128, 6.28),
...
hazards = {0  : ('Acceleration/Gravity', 'Falls'),
           ...
           9  : ('Chemical/Water Contamination',
                 'Backflow/Siphon Effect'),
           10 : ('Chemical/Water Contamination', 'Leaks/Spills'),
           11 : ('Chemical/Water Contamination',
                 'System-Cross Connection'),
           12 : ('Chemical/Water Contamination',
           ...
           178: ('Unannunciated Utility Outages',
                 'Air Conditioning'),
           179: ('Unannunciated Utility Outages',
                 'Compressed Air/Gas'),
           ...
           184: ('Unannunciated Utility Outages',
                 'Lubrication Drains/Sumps'),
           185: ('Unannunciated Utility Outages', 'Steam'),
           186: ('Unannunciated Utility Outages', 'Ventilation')}

stakeholders = {0: ('Customer',)}

statuses = {0 : ('Initiated', 'Incident has been initiated.',
                 'incident'),
            1 : ('Reviewed', 'Incident has been reviewed.',
                 'incident'),
            2 : ('Analysis',
                 'Incident has been assigned and is being analyzed.',
                 'incident'),
            3 : ('Solution Identified',
                 'A solution to the reported problem has been ' \
                 'identified.', 'incident'),
            ...
            6 : ('Ready for Approval',
                 ...
                 'incident'),
            7 : ('Approved', 'Incident analysis has been approved.',
                 'incident'),
            8 : ('Ready for Closure',
                 'Incident is ready to be closed.', 'incident'),
            9 : ('Closed', 'Incident has been closed.', 'incident'),
            10: ('Initiated', 'Action has been initiated.',
                 'action'),
            11: ('Reviewed', 'Action has been reviewed.', 'action'),
            12: ('Approved', 'Action has been approved.', 'action'),
            13: ('Ready for Closure',
                 'Action is ready to be closed.', 'action'),
            14: ('Closed', 'Action has been closed.', 'action')}

conditions = {0 : ('Cavitation', 'operating'),
...
measurements = {0 : ('Contamination, Concentration',),
