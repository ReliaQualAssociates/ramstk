...
class ImportDataModel(RAMSTKDataModel):
    ...
    _dic_field_map = {
        ...
        'Reliability':
        OrderedDict([
            ('Hardware ID', ''), ('Additive Adjustment Factor', ''),
    ...
    def get_db_fields(self, module):
        ...
        try:
            ...
            if module.lower() == 'hardware':
                _db_fields += list(
                    self._dic_field_map['Design Electric'].keys(),
                )[1:]
                _db_fields += list(
                    self._dic_field_map['Reliability'].keys(),
                )[1:]
        except KeyError:
            _db_fields = []
