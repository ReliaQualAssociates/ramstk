...
class DataManager(RAMSTKDataManager):
    ...
    def __init__(self, **kwargs: Dict[str, Any]) -> None:
        ...
        pub.subscribe(self._do_insert_testmethod, "request_insert_pof_testmethod")
    ...
    def do_select_all(self, attributes: Dict[str, Any]) -> None:
        ...
        for _mode in (
            self.dao.session.query(RAMSTKMode)
            .filter(
    ...
    def do_update(self, node_id: int) -> None:
        ...
        try:
            ...
            _error_msg = (
                "{1}: Attempted to save non-existent PoF " "record ID {0}."
            ...
            _method_name: str = inspect.currentframe().f_code.co_name  # type: ignore
            _error_msg = (
                "{1}: No data package found for PoF record " "ID {0}."
            ).format(str(node_id), _method_name)
            pub.sendMessage(
                "do_log_debug",
                logger_name="DEBUG",
            ...
            _method_name: str = inspect.currentframe().f_code.co_name  # type: ignore
            _error_msg = (
                "{1}: The value for one or more attributes for PoF "
                "record ID {0} was the wrong type."
            ).format(str(node_id), _method_name)
            pub.sendMessage(
                "do_log_debug",
                logger_name="DEBUG",
    ...
    def _do_delete(self, node_id: int) -> None:
        ...
        try:
            ...
            _method_name: str = inspect.currentframe().f_code.co_name  # type: ignore
            _error_msg: str = (
                "{1}: Attempted to delete non-existent PoF record " "with ID {0}."
            ).format(str(node_id), _method_name)
            pub.sendMessage(
                "do_log_debug",
                logger_name="DEBUG",
    ...
    def _do_insert_opload(self, parent_id: str) -> None:
        ...
        try:
            ...
            self.last_id["opload"] = max(self.last_id["opload"], _opload.load_id)

            pub.sendMessage(
                "succeed_insert_pof", node_id=self.last_id["opload"], tree=self.tree
            )
            pub.sendMessage(
                "succeed_insert_opload", node_id=_identifier, tree=self.tree
            )
    ...
    def _do_insert_opstress(self, parent_id: str) -> None:
        ...
        try:
            ...
            _identifier = "{0:s}.{1:d}.s".format(parent_id, _opstress.stress_id)
            ...
            self.last_id["opstress"] = max(
                self.last_id["opstress"], _opstress.stress_id
            )

            pub.sendMessage(
                "succeed_insert_pof", node_id=self.last_id["opstress"], tree=self.tree
            )
            pub.sendMessage(
                "succeed_insert_opstress", node_id=_identifier, tree=self.tree
            )
        except (DataAccessError, NodeIDAbsentError):
            _error_msg = (
    ...
    def _do_insert_testmethod(self, parent_id: str) -> None:
        ...
        try:
            ...
            self.last_id["testmethod"] = max(
                self.last_id["testmethod"], _method.test_id
            )

            pub.sendMessage(
                "succeed_insert_pof", node_id=self.last_id["testmethod"], tree=self.tree
            )
            pub.sendMessage(
                "succeed_insert_test_method", node_id=_identifier, tree=self.tree
            )
        except (DataAccessError, NodeIDAbsentError):
            _error_msg = (
                "An error occurred when attempting to add a test "
                "method to operating load "
                "ID {0:s}.".format(_load_id)
            )
            pub.sendMessage("fail_insert_test_method", error_message=_error_msg)
    ...
    def _do_select_all_mechanism(self, mode_id: int) -> None:
        ...
        """
        for _mechanism in (
            self.dao.session.query(RAMSTKMechanism)
            .filter(
                RAMSTKMode.revision_id == self._revision_id,
                RAMSTKMode.hardware_id == self._parent_id,
                RAMSTKMechanism.mode_id == mode_id,
            )
            .all()
        ):

            _identifier = "{0:d}.{1:d}".format(mode_id, _mechanism.mechanism_id)
    ...
    def _do_select_all_opstress(self, parent_id: str) -> None:
        ...
        (_mode_id, _mechanism_id, _load_id) = parent_id.split(".")
        for _opstress in (
            self.dao.session.query(RAMSTKOpStress)
            .filter(
                ...
                RAMSTKOpStress.load_id == int(_load_id),
            )
            .all()
        ):
    ...
    def _do_select_all_testmethod(self, parent_id: str) -> None:
        ...
        (_mode_id, _mechanism_id, _load_id) = parent_id.split(".")
        for _method in (
            self.dao.session.query(RAMSTKTestMethod)
            .filter(
                ...
                RAMSTKTestMethod.load_id == int(_load_id),
            )
            .all()
        ):
