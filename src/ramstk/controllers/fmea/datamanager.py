...
class DataManager(RAMSTKDataManager):
    ...
    def __init__(self, **kwargs: Dict[str, Any]) -> None:
        ...
        pub.subscribe(self._do_insert_mechanism, "request_insert_fmea_mechanism")
    ...
    def do_update(self, node_id: int) -> None:
        ...
        try:
            ...
            _error_msg = (
                "{1}: Attempted to save non-existent FMEA " "record ID {0}."
            ...
            _method_name: str = inspect.currentframe().f_code.co_name  # type: ignore
            _error_msg = (
                "{1}: No data package found for FMEA record " "ID {0}."
            ).format(str(node_id), _method_name)
            pub.sendMessage(
                "do_log_debug",
                logger_name="DEBUG",
    ...
    def _add_cause_node(self, cause: RAMSTKCause, parent_id: str) -> None:
        ...
        :rtype: None
        """
        _identifier = "{0:s}.{1:d}".format(parent_id, cause.cause_id)  # type: ignore

        self.tree.create_node(
            tag="cause", identifier=_identifier, parent=parent_id, data={"cause": cause}
        )

        self.last_id["cause"] = max(
            self.last_id["cause"], cause.cause_id
        )  # type: ignore
    ...
    def _do_insert_control(self, parent_id: str) -> None:
        ...
        try:
            ...
            _identifier = "{0:s}.{1:d}.c".format(parent_id, _control.control_id)
            self.tree.create_node(
                tag="control",
                identifier=_identifier,
                parent=parent_id,
    ...
    def _do_insert_mechanism(self, mode_id: str) -> None:
        ...
        try:
            ...
            _identifier = "{0:s}.{1:d}".format(mode_id, _mechanism.mechanism_id)
    ...
    def _do_insert_mode(self) -> None:
        ...
        try:
            ...
            _error_msg: str = (
                ...
                "record with hardware ID {0}."
            ).format(str(self._root), _method_name)
            pub.sendMessage(
                "do_log_debug",
                logger_name="DEBUG",
                message=_error_msg,
    ...
    def _do_select_all_action(self, parent_id: str) -> None:
        ...
        for _action in (
            ...
            self.last_id["action"] = max(self.last_id["action"], _action.action_id)
    ...
    def _do_select_all_cause(self, parent_id: str) -> None:
        ...
        (_mode_id, _mechanism_id) = parent_id.split(".")
        for _cause in (
            self.dao.session.query(RAMSTKCause)
            .filter(
                RAMSTKCause.mode_id == int(_mode_id),
                RAMSTKCause.mechanism_id == int(_mechanism_id),
            )
            .all()
        ):
    ...
    def _do_select_all_control(self, parent_id: str) -> None:
        ...
        for _control in (
            ...
            self.last_id["control"] = max(self.last_id["control"], _control.control_id)
    ...
    def _do_select_all_functional_fmea(self) -> None:
        ...
        for _mode in (
            ...
            .filter(RAMSTKMode.function_id == self._parent_id)
            .all()
        ):
    ...
    def _do_select_all_mechanism(self, mode_id: int) -> None:
        ...
        for _mechanism in (
            ...
            .filter(RAMSTKMechanism.mode_id == mode_id)
            .all()
        ):
