...
def calculate_part_count(**attributes):
    ...
    """
    return get_part_count_lambda_b_list(
...
def calculate_part_stress(**attributes):
    ...
    (
        attributes['piSR'],
        _error_msg,
    ) = calculate_series_resistance_factor(
...
def calculate_part_stress_lambda_b(
        subcategory_id, temperature_rated_max,
        temperature_active, voltage_ratio,
):
    """
    Calculate the part stress base hazard rate (lambda b) from MIL-HDBK-217F.

    :param int subcategory_id: the capacitor subcategory identifier.
    :param float temperature_rated_max: the maximum rated temperature of the capacitor.
    :param float temperature_active: the operating ambient temperature of the capacitor.
    :param float voltage_ratio: the ratio of operating to rated voltage for the capacitor.
...
def calculate_series_resistance_factor(
        resistance, voltage_dc_operating,
    ...
    try:
        ...
        _error_msg = (
            "RAMTK WARNING: Calculated series resistance factor "
            "(piSR) may be erroneous due to bad inputs.  ESR: {0:f}, DC "
            "operating voltage: {1:f}, ac operating voltage: {2:f} "
        ).format(
            resistance, voltage_dc_operating, voltage_ac_operating,
        )
        _ckt_resistance = 0.01
...
def get_part_count_lambda_b_list(subcategory_id, specification_id=None):
    r"""
    Retrieves the MIL-HDBK-217F parts count base hazard rate (lambda b) list.

    The dictionary PART_COUNT_LAMBDA_B contains the MIL-HDBK-217F parts count
    base hazard rates lists.  Keys are for PART_COUNT_LAMBDA_B are:
    ...
    """
    if subcategory_id == 1:
        _lst_base_hr = PART_COUNT_LAMBDA_B[subcategory_id][specification_id]
    else:
        _lst_base_hr = PART_COUNT_LAMBDA_B[subcategory_id]
