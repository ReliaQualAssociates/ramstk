...
def calculate_rpn(sod):
    ...
    Calculate the Risk Priority Number (RPN).
        ...
        ...
        ramstk.exceptions.OutOfRangeError: RPN severity is outside the range [1, 10].


    :param dict sod: a dict containing the S, O, and D values from which to
        calculate the RPN.
...
def calculate_mode_hazard_rate(item_hr, mode_ratio):
    ...
    :param float item_hr: the hazard rate of the (hardware) item the mode is
        associated with.
    :param float mode_ratio:
...
def calculate_mode_criticality(mode_hr, mode_op_time, eff_prob):
    ...
    :param float mode_hr: the hazard rate of the failure mode.
    :param float mode_op_time: the operating time of the failure mode (i.e.,
        the time at risk).
    :param float eff_prob: the probability the end effect is the one observed.
