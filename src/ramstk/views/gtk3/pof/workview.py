...
class PoFPanel(RAMSTKPanel):
    ...
    def do_set_callbacks(self) -> None:
        ...
        for i in self._lst_col_order:
            _cell = self.tvwTreeView.get_column(self._lst_col_order[i]).get_cells()

            if isinstance(_cell[0], Gtk.CellRendererPixbuf):
                pass
            else:
                _cell[0].connect("edited", super().on_cell_edit, i, "wvw_editing_pof")
...
class PoF(RAMSTKWorkView):
    ...
    def _do_request_insert_child(self, __button: Gtk.ToolButton) -> None:
        ...
        super().do_set_cursor_busy()
        pub.sendMessage(
            "request_insert_pof_{0:s}".format(_level), parent_id=str(_parent_id)
        )
    ...
    def __make_ui(self) -> None:
        ...
        for _idx, _key in enumerate(self.RAMSTK_USER_CONFIGURATION.RAMSTK_LOAD_HISTORY):
        ...
        for _idx, _key in enumerate(
            ...
            self._pnlPanel.dic_measurable_parameters[
                _idx + 1
            ] = self.RAMSTK_USER_CONFIGURATION.RAMSTK_MEASURABLE_PARAMETERS[_key][1]
