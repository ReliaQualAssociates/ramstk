...
class GeneralDataPanel(RAMSTKPanel):
    ...
    def _do_load_panel(self, attributes: Dict[str, Any]) -> None:
        ...
        self.txtCode.do_update(str(attributes["requirement_code"]), signal="changed")
        self.txtName.do_update(str(attributes["description"]), signal="changed")
        self.chkDerived.do_update(int(attributes["derived"]), signal="toggled")
        self.cmbRequirementType.do_update(
            int(attributes["requirement_type"]), signal="changed"
        )
        self.txtSpecification.do_update(
            str(attributes["specification"]), signal="changed"
        )
        self.txtPageNum.do_update(str(attributes["page_number"]), signal="changed")
        self.txtFigNum.do_update(str(attributes["figure_number"]), signal="changed")
        self.cmbPriority.do_update(int(attributes["priority"]), signal="changed")
        self.cmbOwner.do_update(int(attributes["owner"]), signal="changed")
        self.chkValidated.do_update(int(attributes["validated"]), signal="toggled")
        if attributes["validated"]:
            self.txtValidatedDate.do_update(
                str(attributes["validated_date"]), signal="changed"
            )
        else:
            self.txtValidatedDate.do_update("", signal="changed")
    ...
    def __do_set_callbacks(self) -> None:
        ...
        self.btnValidateDate.dic_handler_id["released"] = self.btnValidateDate.connect(
            "button-release-event", self._do_select_date, self.txtValidatedDate
        )

        self.chkDerived.dic_handler_id["toggled"] = self.chkDerived.connect(
            "toggled", super().on_toggled, 2, "wvw_editing_requirement"
        )
        self.chkValidated.dic_handler_id["toggled"] = self.chkValidated.connect(
            "toggled", super().on_toggled, 12, "wvw_editing_requirement"
        )

        self.cmbOwner.dic_handler_id["changed"] = self.cmbOwner.connect(
            "changed", super().on_changed_combo, 5, "wvw_editing_requirement"
        )
        self.cmbPriority.dic_handler_id["changed"] = self.cmbPriority.connect(
            "changed", super().on_changed_combo, 8, "wvw_editing_requirement"
        )
        self.cmbRequirementType.dic_handler_id[
            "changed"
        ] = self.cmbRequirementType.connect(
            "changed", super().on_changed_combo, 11, "wvw_editing_requirement"
        )
        ...
        self.txtFigNum.dic_handler_id["changed"] = self.txtFigNum.connect(
            "changed", super().on_changed_entry, 4, "wvw_editing_requirement"
        )
        self.txtPageNum.dic_handler_id["changed"] = self.txtPageNum.connect(
            "changed", super().on_changed_entry, 6, "wvw_editing_requirement"
        )
        self.txtCode.dic_handler_id["changed"] = self.txtCode.connect(
            "changed", super().on_changed_entry, 9, "wvw_editing_requirement"
        )
        self.txtSpecification.dic_handler_id["changed"] = self.txtSpecification.connect(
            "changed", super().on_changed_entry, 10, "wvw_editing_requirement"
        )
        self.txtValidatedDate.dic_handler_id["changed"] = self.txtValidatedDate.connect(
            "changed", super().on_changed_entry, 13, "wvw_editing_requirement"
        )
    ...
    def __do_set_properties(self) -> None:
        ...
        self.chkDerived.do_set_properties(
            tooltip=_("Indicates whether or not the selected requirement is derived."),
            width=400,
        )  # noqa
        ...
        self.txtCode.do_set_properties(
            width=125, tooltip=_("A unique code for the selected requirement.")
        )
    ...
    def do_set_callbacks(self) -> None:
        ...
        for _index, _checkbutton in enumerate(self._lst_widgets):
            _checkbutton.dic_handler_id["toggled"] = _checkbutton.connect(
                "toggled", super().on_toggled, _index, "wvw_editing_requirement"
            )
...
class ClarityPanel(AnalysisPanel):
    ...
    def _do_load_panel(self, attributes: Dict[str, Any]) -> None:
        ...
        for _index, _checkbutton in enumerate(self._lst_widgets):
            _checkbutton.do_update(
                int(attributes["q_clarity_{0:d}".format(_index)]), signal="toggled"
            )  # noqa
...
class CompletenessPanel(AnalysisPanel):
    ...
    def __init__(self) -> None:
        ...
        self._lst_labels = [
            ...
            _("9. The requirement contains no implied implementation " "constraints."),
            _(
    ...
    def _do_load_panel(self, attributes: Dict[str, Any]) -> None:
        ...
        for _index, _checkbutton in enumerate(self._lst_widgets):
            _checkbutton.do_update(
                int(attributes["q_complete_{0:d}".format(_index)]), signal="toggled"
            )  # noqa
...
class ConsistencyPanel(AnalysisPanel):
    ...
    def _do_load_panel(self, attributes: Dict[str, Any]) -> None:
        ...
        for _index, _checkbutton in enumerate(self._lst_widgets):
            _checkbutton.do_update(
                int(attributes["q_consistent_{0:d}".format(_index)]), signal="toggled"
            )  # noqa
...
class VerifiabilityPanel(AnalysisPanel):
    ...
    def _do_load_panel(self, attributes: Dict[str, Any]) -> None:
        ...
        for _index, _checkbutton in enumerate(self._lst_widgets):
            _checkbutton.do_update(
                int(attributes["q_verifiable_{0:d}".format(_index)]), signal="toggled"
            )  # noqa
